...
import logo from "../../../assets/icons/common/logo_white.svg";
...
export default function HeaderOne() {
  ...
  return (
      <>
        ...
        <header className={`flex gap-[25px] max-[1280px]:gap-[20px] z-[9999] items-center bg-[#0D1F34] py-[30px] px-2 w-full 
            ...
            {isOpen&&
                <div onClick={closeMobileMenu} className={`h-[11000px] fixed z-[99999] top-0 left-0 w-full`}>
                  <div onClick={(event)=>{event.stopPropagation();}}
                       className={"relative h-full drop-shadow-2xl bg-[#0D1F34] w-[75%] pt-[100px] px-[20px]"}>
                    {menuItems.map((item, key) => {
                      return item.hasOwnProperty("children") ? (
                          <DropdownMobile closeMobileMenu={closeMobileMenu} key={key} item={item} />
                      ) : (
                          <div key={key} className={`${isActiveLink(item.hash?item.hash:item.route||"")?"text-blue-500":"text-white"} 
          ...
          <div className="flex gap-[25px] items-center justify-center text-black max-[1280px]:gap-[15px] max-[1180px]:hidden">
            {menuItems.map((item, key) => {
              return item.hasOwnProperty("children") ? (
                  ...
                  <div key={key} className={`${isActiveLink(item.hash?item.hash:item.route||"")?"text-blue-500":"text-white"} hover:text-blue-500`}>
